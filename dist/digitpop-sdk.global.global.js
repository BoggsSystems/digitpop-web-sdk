"use strict";var DigitPopSDK=(()=>{var l=Object.defineProperty;var u=Object.getOwnPropertyDescriptor;var f=Object.getOwnPropertyNames;var m=Object.prototype.hasOwnProperty;var b=(n,t)=>{for(var e in t)l(n,e,{get:t[e],enumerable:!0})},w=(n,t,e,i)=>{if(t&&typeof t=="object"||typeof t=="function")for(let s of f(t))!m.call(n,s)&&s!==e&&l(n,s,{get:()=>t[s],enumerable:!(i=u(t,s))||i.enumerable});return n};var v=n=>w(l({},"__esModule",{value:!0}),n);var x={};b(x,{ApiClient:()=>d,DigitPopClient:()=>o,GatewayModal:()=>p,WebSocketClient:()=>c,createDigitPop:()=>h,default:()=>y});var d=class{constructor(t){this.config=t,this.baseUrl=this.resolveBaseUrl()}getBaseUrl(){return this.baseUrl}resolveBaseUrl(){return this.config.apiUrl?this.config.apiUrl.replace(/\/+$/,""):(this.config.environment==="staging","https://digitpop-server-staging.up.railway.app")}async getWalletBalance(t){let e=t||this.config.userId;try{let i=`${this.baseUrl}/api/engagement/user/${encodeURIComponent(e)}`,s=await fetch(i,{headers:{"X-DigitPop-Publisher-Key":this.config.publicKey}});if(!s.ok)throw new Error(`Failed to fetch wallet balance (HTTP ${s.status})`);let r=await s.json(),a=r.user?.earnedCredits??r.earnedCredits??0;return{earnedCredits:a,availableCredits:a,attentionCredits:a,popCoins:Math.floor(a*10),isCandidateAccount:!0}}catch(i){return console.warn("[DigitPop SDK] Failed to query wallet balance, returning fallback zero:",i.message),{earnedCredits:0,availableCredits:0,attentionCredits:0,popCoins:0,isCandidateAccount:!0}}}async grantAttentionReward(t){let e=await fetch(`${this.baseUrl}/api/engagement/grant-credit`,{method:"POST",headers:{"Content-Type":"application/json","X-DigitPop-Publisher-Key":this.config.publicKey},body:JSON.stringify({userId:t.userId,userEmail:this.config.userMetadata?.email,candidateName:this.config.userMetadata?.fullName,credits:t.credits,publisherKey:this.config.publicKey,videoId:t.videoId,engagementType:"VIDEO_WATCH",watchTimeSeconds:30})});if(!e.ok){let s=await e.json().catch(()=>({}));throw new Error(s.message||`Failed to grant credit (HTTP ${e.status})`)}let i=await e.json();return{success:!0,earnedCredits:i.totalEarnedCredits??i.data?.earnedCredits??t.credits}}async redeemTokens(t){let e=await fetch(`${this.baseUrl}/api/redemption/purchase-with-tokens`,{method:"POST",headers:{"Content-Type":"application/json","X-DigitPop-Publisher-Key":this.config.publicKey},body:JSON.stringify({userId:t.userId,tokenCost:t.tokenAmount,productId:t.assetId,deliveryEmail:this.config.userMetadata?.email})});if(!e.ok){let s=await e.json().catch(()=>({}));throw new Error(s.message||`Failed to redeem tokens (HTTP ${e.status})`)}let i=await e.json();return{success:!0,transactionId:i.redemption?.id||i.transactionId||`tx_redeem_${Date.now()}`}}};var c=class{constructor(t){this.ws=null;this.listeners=new Set;this.isConnecting=!1;this.reconnectTimer=null;this.config=t}resolveWsUrl(){return this.config.wsUrl?this.config.wsUrl.replace(/\/+$/,""):(this.config.environment==="staging","wss://digitpop-server-staging.up.railway.app")}connect(){if(typeof window>"u"||typeof window.WebSocket>"u"||this.ws&&(this.ws.readyState===0||this.ws.readyState===1))return;this.isConnecting=!0;let e=`${this.resolveWsUrl()}/${encodeURIComponent(this.config.userId)}`;try{this.ws=new window.WebSocket(e,`${this.config.userId}-player`),this.ws.onopen=()=>{this.isConnecting=!1,this.send({data:{trigger:"login",value:this.config.userId}})},this.ws.onmessage=i=>{try{let s=JSON.parse(i.data);this.notifyListeners(s)}catch{}},this.ws.onerror=i=>{this.isConnecting=!1},this.ws.onclose=()=>{this.isConnecting=!1,this.ws=null}}catch(i){this.isConnecting=!1,console.warn("[DigitPop SDK] WebSocket connection failed:",i)}}send(t){this.ws&&this.ws.readyState===1&&this.ws.send(typeof t=="string"?t:JSON.stringify(t))}subscribe(t){return this.listeners.add(t),()=>this.listeners.delete(t)}notifyListeners(t){this.listeners.forEach(e=>{try{e(t)}catch(i){console.error("[DigitPop SDK] Error in WebSocket listener:",i)}})}disconnect(){this.reconnectTimer&&clearTimeout(this.reconnectTimer),this.ws&&(this.ws.close(),this.ws=null),this.listeners.clear()}};var p=class{constructor(t,e){this.hostElement=null;this.shadowRoot=null;this.currentStage="SELECTION";this.watchedVideos=0;this.requiredVideos=2;this.creditsPerWatch=10;this.watchTimer=null;this.watchProgress=0;this.handleKeyDown=t=>{t.key==="Escape"&&this.close()};this.client=t,this.options=e;let i=e.monetizationOptions?.find(s=>s.type==="VIDEO_ENGAGEMENT");i&&i.type==="VIDEO_ENGAGEMENT"&&(this.requiredVideos=i.requiredVideos||2,this.creditsPerWatch=i.creditsPerWatch||10)}render(){typeof document>"u"||(this.close(),this.hostElement=document.createElement("div"),this.hostElement.id="digitpop-gateway-root",this.shadowRoot=this.hostElement.attachShadow({mode:"open"}),document.body.appendChild(this.hostElement),this.updateContent(),window.addEventListener("keydown",this.handleKeyDown))}close(){this.watchTimer&&(clearInterval(this.watchTimer),this.watchTimer=null),window.removeEventListener("keydown",this.handleKeyDown),this.hostElement&&this.hostElement.parentNode&&this.hostElement.parentNode.removeChild(this.hostElement),this.hostElement=null,this.shadowRoot=null,this.options.onClose&&this.options.onClose()}updateContent(){this.shadowRoot&&(this.shadowRoot.innerHTML=`
      <style>
        ${this.getStyles()}
      </style>
